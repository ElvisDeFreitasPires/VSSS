...
# G = np.array([1000, screen_dim[1]/2,np.deg2rad(-90)])
G = np.array([320, screen_dim[1]/2,np.deg2rad(-90)])
...
while(True):
 ...
 #   _,x_r3,y_r3,th_r3 = detecta_robo(frame,cor_amarelo,cor_verde)
    ...
    if sim_running:
        ...
        if abs(v) < 10 :
            if v < 0:
                env_vLvW = '-0' + str(int(abs(v)))
            else:
                env_vLvW = '0' + str(int(v))
        ...
        # P = P + dPdt.flatten() * dt # subiu para atualizar mais rapido com a cap
                                      # captura da imagem

        P[2] = ajustaangulo(th_r1)
        hud(P)
